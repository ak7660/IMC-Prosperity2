...
class Trader:
    def __init__(self):
        ...
        self.position_limit_Dic = {'STARFRUIT' : 20, 'AMETHYSTS' : 20, 'CHOCOLATE' : 250, 'STRAWBERRIES' : 350, 'ROSES' : 60, 'ORCHIDS' : 100, 'GIFT_BASKET' : 60, 'COCONUT' : 300, 'COCONUT_COUPON' : 600}
        self.order_size_Dic = {'STARFRUIT' : 20, 'AMETHYSTS' : 20, 'CHOCOLATE' : 250, 'STRAWBERRIES' : 350, 'ROSES' : 60, 'ORCHIDS' : 100, 'GIFT_BASKET' : 60, 'COCONUT' : 300, 'COCONUT_COUPON' : 600}
        ...
        self.spread_window = RollingWindow2(50)
        self.saved_z_score = 0
    ...
    def run(self, state: TradingState) -> (dict[Symbol, list[Order]], int, str):
        ...
        for product in ['COCONUT_COUPON', 'COCONUT', 'CHOCOLATE', 'STRAWBERRIES', 'ROSES', 'ORCHIDS', 'STARFRUIT', 'AMETHYSTS', 'GIFT_BASKET']:
            ...
            best_ask2 = self.best_ask(state, 'COCONUT')
            best_bid2 = self.best_bid(state, 'COCONUT')
            position2 = state.position.get('COCONUT', 0)
            latest_own_trade_price = self.get_latest_own_trade_price(state, product)
            
            #Round4
            if product == 'COCONUT_COUPON':
                z_score = self.calculate_spread_z_score(state)
                print('zscore,', z_score)
                z_score_buy_threshold = -2  # Buy COCONUT and sell COCONUT_COUPON if z-score is less than -2
                z_score_sell_threshold = 2  # Sell COCONUT and buy COCONUT_COUPON if z-score is greater than 2
                print(z_score < z_score_buy_threshold)
                print(position + self.order_size_Dic[product] <= self.position_limit_Dic[product])
                print(position >= 0)
                if z_score > z_score_sell_threshold and position + self.order_size_Dic[product] <= self.position_limit_Dic[product] and position >= 0:
                    buy_order = Order(product, best_ask, self.order_size_Dic[product])
                    orders.append(buy_order)
                elif z_score > z_score_sell_threshold and position < self.position_limit_Dic[product]:
                    buy_order = Order(product, best_ask, self.position_limit_Dic[product] - position)
                    orders.append(buy_order)
                            
                if z_score < z_score_buy_threshold and position - self.order_size_Dic[product] >= -self.position_limit_Dic[product] and position <= 0:
                    sell_order = Order(product, best_bid, -self.order_size_Dic[product] - position)
                    orders.append(sell_order)
                elif z_score < z_score_buy_threshold and position > -self.position_limit_Dic[product]:
                    sell_order = Order(product, best_bid, -self.position_limit_Dic[product] - position)
                    orders.append(sell_order)
                
                self.saved_z_score = z_score
            ...
            if product == 'COCONUT':
                ...
                best_ask = self.best_ask(state, product)
                best_bid = self.best_bid(state, product)
                if current_mid_price <= lower_channel and position + self.order_size_Dic[product] <= self.position_limit_Dic[product] and position >= 0 and lower_channel != 0:
                    buy_order = Order(product, int(current_mid_price), self.order_size_Dic[product])
                    orders.append(buy_order)
                elif current_mid_price <= lower_channel and position < self.position_limit_Dic[product] and lower_channel != 0:
                    buy_order = Order(product, int(current_mid_price), self.position_limit_Dic[product] - position)
                    orders.append(buy_order)
                        
                        
                if current_mid_price >= upper_channel and position - self.order_size_Dic[product] >= -self.position_limit_Dic[product] and position <= 0 and upper_channel != 0:
                    sell_order = Order(product, int(current_mid_price), -self.order_size_Dic[product] - position)
                    orders.append(sell_order)
                elif current_mid_price >= upper_channel and position > -self.position_limit_Dic[product] and upper_channel != 0:
            ...
            if product == 'ROSES':
                ...
                if current_mid_price <= lower_channel and position + self.order_size_Dic[product] <= self.position_limit_Dic[product] and position >= 0 and lower_channel != 0:
                    buy_order = Order(product, int(current_mid_price), self.order_size_Dic[product])
                    orders.append(buy_order)
                elif current_mid_price <= lower_channel and position < self.position_limit_Dic[product] and lower_channel:
                    buy_order = Order(product, int(current_mid_price), self.position_limit_Dic[product] - position)
                    orders.append(buy_order)
            ...
            if product == 'ORCHIDS':
                ...
                if position >= -self.arbit_limit and position <= 0 and import_tariff <= -3.5:
                    sell_order = Order(product, int(current_mid_price-1), -self.arbit_limit)
                    orders.append(sell_order)
                elif position >= -self.arbit_limit and import_tariff <= -3.5:
                    sell_order = Order(product, int(current_mid_price-1), -self.arbit_limit)
                    orders.append(sell_order)
    ...
    def calculate_spread_z_score(self, state: TradingState):
        """Calculate the z-score of the price spread between COCONUT and COCONUT_COUPON."""
        coconut_price = self.calculate_vwap_mid_price(state, 'COCONUT')
        coconut_coupon_price = self.calculate_vwap_mid_price(state, 'COCONUT_COUPON')
        spread = coconut_price - coconut_coupon_price
        self.spread_window.push(spread)
        return self.spread_window.get_zscore()
