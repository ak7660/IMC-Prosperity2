    ...
    def run(self, state: TradingState) -> (dict[Symbol, list[Order]], int, str):
        ...
        for product in ['STARFRUIT', 'AMETHYSTS']:
            ...
            if product == 'STARFRUIT':
                ...
                if best_ask < buy_threshold and position + self.order_size <= self.position_limit:
                    buy_order = Order(product, best_ask, self.order_size)
                    orders.append(buy_order)

                if best_bid > sell_threshold and position - self.order_size >= -self.position_limit:
                    sell_order = Order(product, best_bid, -self.order_size)
                    orders.append(sell_order)

            if product == 'AMETHYSTS':
                if best_ask < self.amethysts_threshold_buy and position + self.order_size <= self.position_limit:
                    buy_order = Order(product, best_ask, self.order_size)
                    orders.append(buy_order)
                
                if best_bid > self.amethysts_threshold_sell and position - self.order_size >= -self.position_limit:
                    sell_order = Order(product, best_bid, -self.order_size)
                    orders.append(sell_order)
